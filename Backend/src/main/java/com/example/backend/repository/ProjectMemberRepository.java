...
public interface ProjectMemberRepository extends JpaRepository<ProjectMember, Long> {
    ...
    @Query(value = "SELECT p.* FROM project_member p WHERE p.is_admin = TRUE AND p.project_id = ?1", nativeQuery = true)
    Optional<ProjectMember> findByIsAdminAndProjectId(Long projectId);

    ProjectMember findFirstByProject(Project project);
}
